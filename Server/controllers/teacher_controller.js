...
export const create_attendance =async(req,res)=>{
    ...
    let teacher_id = req.user?.id
let {
student_id,class_id,status,subject_id
} = req.body
    try {
        
let sql = `
INSERT INTO att_db(student_id,class_id,subject_id,teacher_id,att_date,status) VALUES(?,?,?,?,CURDATE(),?)
`

let [data] = await pool.query(sql,[student_id,class_id,subject_id,teacher_id,status])
